...
exports.bookDeleteGet = asyncHandler(async (req, res, next) => {
  ...
  // Get details of book and their related books (in parallel)
  const [book, bookInstances] = await Promise.all([
    Book.findById(bookId).exec(),
    BookInstance.find({ book: bookId }, "imprint _id").exec(),
  ]);
...
exports.bookDeletePost = asyncHandler(async (req, res, next) => {
  ...
  if (bookInstances.length > 0) {
    // Book has books associated. Render in same way as for GET route.
    res.render("bookDelete", {
      title: "Delete Book",
      book,
      bookInstances,
...
exports.bookUpdateGet = asyncHandler(async (req, res, next) => {
  ...
  const [book, allAuthors, allGenres] = await Promise.all([
    Book.findById(bookId).populate("author").exec(),
    Author.find().sort({ family_name: 1 }).exec(),
    Genre.find().sort({ name: 1 }).exec(),
  ]);
...
exports.bookUpdatePost = [
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      // Data from form is valid. Upda  te the record.
